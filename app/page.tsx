              ...
              {showPinInput && (
                <motion.div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md">
                   <motion.div animate={{ y: keyboardOpen && activeInput === "vipPin" ? -180 : 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="bg-white/10 p-10 rounded-[40px] border border-white/20 text-center w-[400px]">
                      ...
                      <input type="password" value={vipPin} onFocus={() => { setActiveInput("vipPin"); setKeyboardOpen(true); }} onKeyDown={playBeep} onChange={(e) => { setVipPin(e.target.value); if (keyboardRef.current && activeInput === "vipPin") keyboardRef.current.setInput(e.target.value); }} className="w-full bg-white/5 border border-white/20 rounded-2xl p-4 text-white text-3xl text-center mb-6 focus:border-amber-500 outline-none transition-all" placeholder="******" />
            ...
            <div className="flex w-full bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[40px] shadow-2xl overflow-hidden">
              <div className="flex-1 p-16 flex flex-col items-center justify-center text-center border-r border-white/10">
                ...
                <div className="mt-12 flex flex-row gap-6 justify-center">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowPhotobooth(true)} className="px-8 py-5 bg-blue-600 text-white text-xl font-bold rounded-2xl shadow-lg shadow-blue-900/20 hover:bg-blue-500 transition-all flex items-center gap-3"><span className="text-2xl">📸</span> Buka Photobooth</motion.button>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setStep(4)} className="px-8 py-5 bg-green-600 text-white text-xl font-bold rounded-2xl shadow-lg shadow-green-900/20 hover:bg-green-500 transition-all">Selesai & Beri Rating</motion.button>
                </div>
              </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md p-10">
            <div className="bg-white/10 p-8 rounded-[40px] border border-white/20 shadow-2xl flex gap-10 max-w-6xl w-full">
              ...
              <div className="w-[350px] flex flex-col justify-center text-center">
                <h2 className="text-3xl font-bold text-white mb-4">Telkom Photobooth</h2><p className="text-gray-400 mb-8">Buat kenang-kenangan kunjungan Anda hari ini!</p>
                {!photoboothResult ? (
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <div className="p-4 bg-white rounded-2xl shadow-lg w-[232px] h-[232px] flex items-center justify-center">
                      {isUploadingPhoto ? (
                        <div className="flex flex-col items-center gap-3"><div className="animate-spin w-10 h-10 border-4 border-red-500 border-t-transparent rounded-full"></div><span className="text-gray-500 font-semibold text-sm animate-pulse">Menyiapkan QR Code...</span></div>
                      ) : photoboothUrl ? (
                        <QRCodeCanvas value={photoboothUrl} size={200} level="H" />
                      ) : (<span className="text-red-500 text-sm font-bold">Gagal memuat QR</span>)}
                    </div>
                    <div className="text-center">
                      {isUploadingPhoto ? (<p className="text-sm text-yellow-400 font-semibold">Sedang mengunggah ke server Telkom...</p>) : photoboothUrl ? (<p className="text-sm text-green-400 font-bold">Scan QR di atas untuk menyimpan foto ke HP Anda!</p>) : (<p className="text-sm text-red-400">Terjadi kesalahan jaringan.</p>)}
                    </div>
