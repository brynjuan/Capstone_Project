...
export function ProblemPanel({ series }: any) {
  ...
  return (
    <section className="rounded-2xl border border-[#f0dfdb] bg-white p-5 shadow-[0_16px_42px_rgba(70,31,25,0.06)] backdrop-blur-2xl">
      <div className="flex items-center justify-between">
        <div><h3 className="text-lg font-bold">Permasalahan Paling Sering</h3></div>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {series.map((item: any, index: number) => (
                <div key={item.name} className="inline-flex items-center gap-2 text-sm font-semibold text-[#725b56]">
                  <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: colors[index % colors.length] }} /> {item.name}
                </div>
              ))}
            </div>
          </>
